...
fn get_guard_data() -> HashMap<u32, Guard> {
    ...
    for x in event_log.into_sorted_vec().iter() {
        if x.guard_id > 0 {
            id = x.guard_id;
            let guard_st: Guard = Guard {
                id,
                log: vec![],
                sleep_freq: HashMap::new(),
                sleep: 0,
            };
            ...
            match guards_sum.get_mut(&id) {
                Some(guard) => {
                    ...
                    for minute_tic in 0..minutes {
                        let tics =
                            (temp_date_time + chrono::Duration::minutes(minute_tic)).minute();
        ...
        match guards_sum.get_mut(&id) {
            Some(guard) => guard.log.push(Event {
                date: x.date,
                guard_id: x.guard_id,
...
fn part1(guard_data: &HashMap<u32, Guard>) -> (String, u32) {
    ...
    let description = format!(
        "Guard id {}, most likely sleeping at 00:{}, was sleeping {} minutes totaly",
        guard_id, minute_sleep, total_sleep
    );
...
fn part2(guard_data: &HashMap<u32, Guard>) -> (String, u32) {
    ...
    let description = format!(
        "Guard id {}, was sleeping at 00:{}, {} times",
        most_minute_guard[0], most_minute_guard[1], most_minute_guard[2]
    );
